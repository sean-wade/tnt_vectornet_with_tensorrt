...
#pragma unroll
    for (int32_t it = 0; it < VPT; it++)
    {
        localX[it] = static_cast<OP_T>(localGamma[it]) * (static_cast<OP_T>(localX[it]) - mu) * rsigma
    ...
    int32_t const nHiddenDimension, T const* input, T const* gamma, T const* beta, T* output, float const epsilon)
{
    ...
    for (int32_t i = threadIdx.x; i < nHiddenDimension; i += TPB)
    {
        int32_t const index = offset + i;
        output[index] =
            (static_cast<OP_T>(output[index]) - mu) * rsigma * static_cast<OP_T>(gamma[i]) + static_cast<OP_T>(beta[i]);
    }
}
...
template int
computeLayerNorm<half>(int const, int const, half const*, half const*, half const*, half*, float const, cudaStream_t);
