    ...
    cudaStream_t            stream) noexcept
{
    ...
    //     std::cout << "inputDesc[0].dims[" << i << "] = " << inputDesc[0].dims.d[i] << "\n";

    // for (int32_t i = 2; i < inputDesc[0].dims.nbDims; ++i)
    for (int32_t i = 1; i < inputDesc[0].dims.nbDims; ++i) { nHiddenSize *= inputDesc[0].dims.d[i]; }
    int32_t status = -1;
    ...
    case DataType::kFLOAT:
    {
        ...
        // float *h_gamma = (float *)malloc(nHiddenSize * sizeof(float));
        // cudaMemcpy(h_gamma, gamma, nHiddenSize * sizeof(float), cudaMemcpyDeviceToHost);
        // for (int i = 0; i < nHiddenSize; i++)
        //     printf(" %f ", h_gamma[i]);

        // float *h_beta = (float *)malloc(10 * sizeof(float));
        // cudaMemcpy(h_beta, beta, 10 * sizeof(float), cudaMemcpyDeviceToHost);
        // for (int i = 0; i < 10; i++)
        //     printf(" %f ", h_beta[i]);
    ...
    case DataType::kINT8:
    {
        ...
        status =
            computeLayerNormQDQ(gridSize, nHiddenSize, input, gamma, beta, output, dqScaleIn, qScale, mEpsilon, stream);
        break;
    }
    default:
    {
...
IPluginV2*
LayerNormPluginCreator::deserializePlugin(char const* name, void const* serialData, size_t serialLength) noexcept
{
    try
    {
