/*
 * @Author: zhanghao
 * @LastEditTime: 2023-03-24 15:38:13
 * @FilePath: /vectornetx/Plugin/layerNormPlugin/layerNormKernel.cu
...
__global__ void
LayerNormSmallKernel(int32_t const nHiddenDimension, T const* input, T const* gamma, T const* beta, T* output, float const epsilon)
{
    ...
    if (threadIdx.x < nHiddenDimension)
    {
        ...
        OP_T const g = 1.0, b = 0.0;
        output[index] = (val - mu) * rsigma * g + b;
    }
}

template __global__ void
LayerNormSmallKernel<float, float, 32>(int32_t const, float const*, float const*, float const*, float*, float const);

template __global__ void
LayerNormSmallKernel<__half, float, 32>(int32_t const, __half const*, __half const*, __half const*, __half*, float const);

template <typename T, typename OP_T, int32_t TPB, int32_t VPT>
__global__ void
LayerNormMediumKernel(int32_t const nHiddenDimension, T const* input, T const* gamma, T const* beta, T* output, float const epsilon)
{
...
template __global__ void
LayerNormMediumKernel<float, float, 64, 4>(int32_t const, float const*, float const*, float const*, float*, float const);
template __global__ void
LayerNormMediumKernel<__half, float, 64, 4>(int32_t const, __half const*, __half const*, __half const*, __half*, float const);

template <typename T, typename OP_T, int32_t TPB>
__global__ void
LayerNormLargeKernel(int32_t const nHiddenDimension, T const* input, T const* gamma, T const* beta, T* output, float const epsilon)
{
...
template __global__ void
LayerNormLargeKernel<float, float, 256>(int32_t const, float const*, float const*, float const*, float*, float const);
template __global__ void
LayerNormLargeKernel<__half, float, 256>(int32_t const, __half const*, __half const*, __half const*, __half*, float const);

template <int32_t TPB, int32_t VPT>
__global__ void LayerNormQDQKernel(
...
template __global__ void
LayerNormQDQKernel<32, 8>(int32_t const, int8_t const*, int8_t*, __half const*, __half const*, float const, float const, float const);
template __global__ void
LayerNormQDQKernel<128, 8>(int32_t const, int8_t const*, int8_t*, __half const*, __half const*, float const, float const, float const);
    ...
    cudaStream_t  stream)
{
    ...
    if (nHiddenDimension <= 32)
    {
        ...
        (LayerNormSmallKernel<T, float, TPB>)<<<gridSize, TPB, 0, stream>>>(nHiddenDimension, input, gamma, beta, output, epsilon);
    }
    else if (nHiddenDimension == 320)
    {
        constexpr int32_t TPB = 320 / VPT;
        (LayerNormMediumKernel<T, float, TPB, VPT>)<<<gridSize, TPB, 0, stream>>>(nHiddenDimension, input, gamma, beta, output, epsilon);
    }
    ...
    else
    {
        ...
        (LayerNormLargeKernel<T, float, TPB>)<<<gridSize, TPB, 0, stream>>>(nHiddenDimension, input, gamma, beta, output, epsilon);
    }
    PLUGIN_CHECK_CUDA(cudaPeekAtLastError());
    return 0;
}

template int computeLayerNorm<float>(int const, int const, float const*, float const*, float const*, float*, float const, cudaStream_t);

template int computeLayerNorm<half>(int const, int const, half const*, half const*, half const*, half*, float const, cudaStream_t);
