/*
 * @Author: zhanghao
 * @LastEditTime: 2023-03-24 15:37:47
 * @FilePath: /vectornetx/Plugin/layerNormPlugin/layerNormPlugin.cpp
...
LayerNormPlugin::LayerNormPlugin(std::string const& name, float mEpsilon) : mName(name), mEpsilon(mEpsilon)
{
}

LayerNormPlugin::LayerNormPlugin(std::string const& name, void const* buffer, size_t length) : mName(name)
{
...
DimsExprs
LayerNormPlugin::getOutputDimensions(int32_t outputIndex, DimsExprs const* inputs, int32_t nbInputs, IExprBuilder& exprBuilder) noexcept
{
    return inputs[0];
}

bool LayerNormPlugin::supportsFormatCombination(int32_t pos, PluginTensorDesc const* inOut, int32_t nbInputs, int32_t nbOutputs) noexcept
{
    switch (pos)
    {
    case 0:
...
size_t
LayerNormPlugin::getWorkspaceSize(PluginTensorDesc const* inputs, int32_t nbInputs, PluginTensorDesc const* outputs, int32_t nbOutputs)
    const noexcept
{
    return 0;
}

int32_t LayerNormPlugin::enqueue(
    ...
    cudaStream_t            stream) noexcept
{
    ...
    for (int32_t i = 1; i < inputDesc[0].dims.nbDims; ++i)
    {
    ...
    case DataType::kINT8:
    {
        ...
        status = computeLayerNormQDQ(gridSize, nHiddenSize, input, gamma, beta, output, dqScaleIn, qScale, mEpsilon, stream);
        break;
    }
    default:
    {
...
IPluginV2* LayerNormPluginCreator::deserializePlugin(char const* name, void const* serialData, size_t serialLength) noexcept
{
    try
    {
