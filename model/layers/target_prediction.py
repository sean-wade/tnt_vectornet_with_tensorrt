...
class TargetPred(nn.Module):
    ...
    def forward(self, feat_in: torch.Tensor, tar_candidate: torch.Tensor):
        ...
        :param feat_in:        the encoded trajectory features, [1, 64]
        :param tar_candidate:  the target position candidate (x, y), [N, 2]
        ...
        n, _ = tar_candidate.size()

        # stack the target candidates to the end of input feature
        feat_in_repeat = torch.cat([feat_in.repeat(n, 1), tar_candidate], dim=1)
