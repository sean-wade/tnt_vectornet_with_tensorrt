...
class Spline2D:
    ...
    @staticmethod
    def remove_dup(x, y):
        x, idx_x = np.unique(x, return_index=True)
        y = y[idx_x]
        y, idx_y = np.unique(y, return_index=True)
        x = x[idx_y]
        return x, y
...
def main():  # pragma: no cover
    ...
    x = [-2.51, 0.02, 2.44, 5.03, 7.58, 10.2, 12.5]
    y = [0.72, 0.63, 0.51, 0.65, 0.66, 0.55, 0.719]
...
if __name__ == '__main__':
    ...
    # y = [0.72, 0.63, 0.51, 0.65, 0.66, 0.55, 0.719]
    sp = Spline2D(x, y)
    print(sp.x_fine, (len(sp.x_fine)))
    print(sp.y_fine, (len(sp.y_fine)))
